...
if ($_POST) {
    ...
    try {
        $stmt = $pdo->prepare("SELECT id, password, full_name, role FROM users WHERE email = ?");
        $stmt->execute([$email]);
        $user = $stmt->fetch();
        ...
        if ($user && password_verify($password, $user['password'])) {
            ...
            if ($user['role'] == 'admin') {
                ...
                header('Location: staff_simple.php');
            } else {
...
<html>
<head>
    <title>Login - Osaze Energy</title>
    <style>
        body { font-family: Arial, sans-serif; background: #f5f5f5; margin: 0; padding: 50px; }
        .login-container { max-width: 400px; margin: 0 auto; background: white; padding: 30px; border-radius: 10px; box-shadow: 0 0 20px rgba(0,0,0,0.1); }
